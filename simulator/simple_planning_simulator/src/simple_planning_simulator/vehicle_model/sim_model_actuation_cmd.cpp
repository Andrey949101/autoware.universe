...
double SteerMap::getSteer(const double steer_rate, const double steer) const
{
  ...
  return interpolation::lerp(steer_rate_interp, output_index_, clamped_steer_rate);
}
  ...
  steer_bias_(steer_bias)
{
  ...
  accel_map_.readAccelMapFromCSV(accel_map_path);
  brake_map_.readBrakeMapFromCSV(brake_map_path);
  steer_map_.readSteerMapFromCSV(steer_map_path);
}
  ...
  const Eigen::VectorXd & state, const Eigen::VectorXd & input)
{
  ...
  const double acc_des = std::clamp(
    accel > 0.0 ? accel_map_.getAcceleration(accel, vel) : brake_map_.getAcceleration(brake, vel),
  ...
  const double time_constant = accel > 0.0 ? accel_time_constant_ : brake_time_constant_;

  const double steer_des = std::clamp(
    steer_map_.getSteer(input(IDX_U::SLOPE_ACCX), steer), -steer_rate_lim_, steer_rate_lim_);
  ...
  d_state(IDX::ACCX) = -(acc - acc_des) / time_constant;
