...
cv::Mat SegmentPointCloudFusionNode::rle_decompress(const std::vector<uint8_t> & rle_data, const int rows, const int cols)
{
  ...
  for (size_t i = 0; i < rle_data.size(); i += step) {
    ...
    int length = *reinterpret_cast<const int *>(&rle_data[i + sizeof(uint8_t)]);
    for (int j = 0; j < length; ++j) {
      int row_idx = static_cast<int>(idx / rows);
